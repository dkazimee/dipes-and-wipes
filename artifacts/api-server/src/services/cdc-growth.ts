...
/**
 * Girls stature-for-age (CDC 2000). Recumbent length 0–23 m; standing height 24–36 m.
 * Values sourced from CDC clinical growth charts (NCHS 2000 data).
 */
...
/**
 * Boys stature-for-age (CDC 2000). Recumbent length 0–23 m; standing height 24–36 m.
 * Values sourced from CDC clinical growth charts (NCHS 2000 data).
 */
...
function weightTable(gender: Gender): CdcWeightPoint[] {
  ...
  // Neutral: average both tables row-by-row
...
function bracketWeight(ageMonths: number, gender: Gender): {
  ...
  // Within data range: standard interpolation
  ...
  // Before data: clamp to first point
  if (ageMonths <= data[0].ageMonths) {
    const first = data[0];
    return { lower: first, upper: first, t: 0 };
  }

  // Beyond data range: linear extrapolation using the last interval's growth rate
  ...
  const t = (ageMonths - last.ageMonths) / interval;
  return { lower: last, upper: extrapolated, t };
}
...
function calcPercentileFromCurve(
  ...
  if (measurement <= curve[0].val) {
    return Math.max(2, 5 * (measurement / curve[0].val));
  }
  if (measurement >= curve[curve.length - 1].val) {
    return 98;
  }
...
/**
 * Given an age in months, a percentile (0–100), and an optional gender,
 * predict weight in lbs using gender-appropriate CDC reference data.
 */
...
/**
 * Given a baby's age in months, actual weight in lbs, and optional gender,
 * compute the CDC percentile (2–98) for weight-for-age.
 */
export function calcPercentileForWeight(ageMonths: number, weightLbs: number, gender?: Gender): number {
  const { lower, upper, t } = bracketWeight(ageMonths, gender);
  const curve = CDC_PERCENTILES.map(p => ({
    pct: p,
    val: lerp(lower.weights[p], upper.weights[p], t),
  }));
...
/**
 * Given an age in months, a percentile (0–100), and an optional gender,
 * predict height in inches using gender-appropriate CDC reference data.
 */
...
/**
 * Given a baby's age in months, actual height in inches, and optional gender,
 * compute the CDC percentile (2–98) for height-for-age.
 */
export function calcPercentileForHeight(ageMonths: number, heightIn: number, gender?: Gender): number {
  const { lower, upper, t } = bracketHeight(ageMonths, gender);
  const curve = CDC_PERCENTILES.map(p => ({
    pct: p,
    val: lerp(lower.heights[p], upper.heights[p], t),
  }));
  return calcPercentileFromCurve(heightIn, curve);
}

/**
 * Calculate a baby's age in months from their birth date (as of today).
 */
...
/**
 * Calculate a baby's age in months at a specific target date.
 */
...
/**
 * Predict the diaper size a baby will need on a given future (or current) date,
 * using the CDC growth curve for the baby's tracked percentile.
 *
 * Falls back through three tiers:
 *   1. Tracked weight percentile (from past measurements)
 *   2. Percentile derived from currentWeightLbs at current age
 *   3. 50th-percentile (CDC median) — no data at all
 */
export function predictDiaperSizeAtDate(
  ...
  // Inline size ranges (mirrors calcDiaperSize in babies.ts)
